...
function UserJuniorHigh(params) {
  const [name, setName] = useState("Parada National High School");
  const [education, setEducation] = useState("");
  const [periodFrom, setPeriodFrom] = useState("2012");
  const [periodTo, setPeriodTo] = useState("2016");
  const [highest, setHighest] = useState("");
  const [yearGraduate, setYearGraduate] = useState("2016");
